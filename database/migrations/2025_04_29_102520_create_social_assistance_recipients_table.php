...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('social_assistance_recipients', function (Blueprint $table) {
            ...
            $table->foreign('social_assistance_id')->references('id')->on('social_assistance');
            $table->uuid('head_of_family_id');
            $table->foreign('head_of_families_id')->references('id')->on('head_of_families');
            ...
            $table->string('proof');
            $table->enum('status',['pending','approved','rejecteed'])->default('pending');
            $table->softDeletes();
            $table->timestamps();
        });
    }
